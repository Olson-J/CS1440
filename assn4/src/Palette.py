...
"""
juliaPalette = ['#ffe4b5', '#ffe5b2', '#ffe7ae', '#ffe9ab', '#ffeaa8', '#ffeda4',
...
def palette(count, type):
    """
    take in an iteration count number
    make sure that count is not larger than length of array
        if ok, return the color

    SOURCE NOTE ABT HOW IT WOULD BE NICE TO BE ABLE TO MORE COLORS BUT CALCULATING THEM IS HARD, IDK WHAT THAT MEANS
    """
    if type == 'julia':
        if count < len(juliaPalette):
            return juliaPalette[count]
        else:
            return juliaPalette[77]
    else:
        if count < len(mbrotPalette):
            return mbrotPalette[count]
        else:
            return mbrotPalette[110]
